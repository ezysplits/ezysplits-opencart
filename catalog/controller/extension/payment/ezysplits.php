...
class ControllerExtensionPaymentEzysplits extends Controller
{
    ...
    public function index()
    {
        ...
        $data['grand_total'] = number_format((float)$order_info['total']);
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/ezysplits')) {
            return $this->load->view($this->config->get('config_template') . '/template/extension/payment/ezysplits', $data);
        } else {
            return $this->load->view('/extension/payment/ezysplits', $data);
        }
    }
    ...
    public function confirm()
    {
        if ($this->session->data['payment_method']['code'] == 'ezysplits') {
            ...
            $appId = trim($this->config->get('payment_ezysplits_merchant_id'));
            $secretKey = trim($this->config->get('payment_ezysplits_secret_key'));
    ...
    public function createCheckoutRequest($order)
    {
        $data = array(
            ...
            'order_meta'        => array(
                'redirect_url' => $this->url->link('extension/payment/ezysplits/thankyou', '', 'SSL'),
                'notify_url' => $this->url->link('extension/payment/ezysplits/callback', '', 'SSL'),
                'cancel_url'       => $this->url->link('extension/payment/ezysplits/cancelled', '', 'SSL'),
            ),
        );
    ...
    private function processResponse($postData)
    {
        ...
        if ($order_info) {
            ...
            if ($postData["signature"] != $computedSignature) {
                $this->model_checkout_order->addOrderHistory($ezysplits_response['order_id'], 10, $postData["message"] . ' Signature missmatch! Check Ezysplits dashboard for details of Reference Id:' . $postData['reference_id']);
            ...
            if ($ezysplits_response["transaction_status"] == 'SUCCESS') {
                if ($order_info["order_status_id"] != $this->config->get('payment_ezysplits_order_status_id')) {
                    // only updated if it has been updated it
                    $this->model_checkout_order->addOrderHistory($ezysplits_response['order_id'], $this->config->get('payment_ezysplits_order_status_id'), "Payment Received");
                }
                return array("status" => 1);
            } else if ($ezysplits_response["transaction_status"] == "CANCELLED") {
                $this->model_checkout_order->addOrderHistory($ezysplits_response['order_id'], 7, $postData["message"] . ' Payment Cancelled! Check Ezysplits dashboard for details of Reference Id:' . $ezysplits_response['reference_id']);
                ...
                $this->model_checkout_order->addOrderHistory($ezysplits_response['orderId'], 10, $postData["message"] . ' Payment Failed! Check Ezysplits dashboard for details of Reference Id:' . $ezysplits_response['reference_id']);
                $redirectUrl = $this->url->link('checkout/failure', '', true);
